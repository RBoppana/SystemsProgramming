...
void freeListing(Listing* listing){
  ...
  free(listing->num_critic_for_reviews);
  free(listing->duration);
  free(listing->director_facebook_likes);
  free(listing->actor_3_facebook_likes);
  free(listing->actor_2_name);
  free(listing->actor_1_facebook_likes);
  free(listing->gross);
  free(listing->genres);
  free(listing->actor_1_name);
  free(listing->movie_title);
  free(listing->num_voted_users);
  free(listing->cast_total_facebook_likes);
  free(listing->actor_3_name);
  free(listing->facenumber_in_poster);
  free(listing->plot_keywords);
  free(listing->movie_imdb_link);
  free(listing->num_user_for_reviews);
  free(listing->language);
  free(listing->country);
  free(listing->content_rating);
  free(listing->budget);
  free(listing->title_year);
  free(listing->actor_2_facebook_likes);
  free(listing->imdb_score);
  free(listing->aspect_ratio);
  free(listing->movie_facebook_likes);
  free(listing->COI);
  free(listing->row);
  free(listing);
}
